...
export function FenToArray (fen: string) {
    ...
    if (fen.length === 0) return new FenParseError("Provided empty FEN string");
    ...
    while (true) {
        ...
        // If character is '/', decrement the row ID.
            ...
            if (charCode === 47 && rowIdx === 0) {
                return new FenParseError("Could not Parse the provided FEN string: Unxpected '/'")
            } else if (charCode === 47) {
                rowIdx -= 1;
                colIdx = 0;
                endOfRank = false;
            } else return new FenParseError("Could not Parse the provided FEN string: Expected '/'")
        }
        ...
        // Increment square if fen character is a number.
            ...
            if (colIdx > 8) {
                return new FenParseError("Could not Parse the provided FEN string: Number '" + fen[idx] + "' is too large.");
            } else if (colIdx === 8) endOfRank = true;
        }

        else return new FenParseError("Could not Parse the provided FEN string: Unexpected symbol: '" + fen[idx] + "'.")
        idx += 1;
    }
